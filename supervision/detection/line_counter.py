...
class LineZone:
    ...
    def trigger(self, detections: Detections) -> Tuple[np.ndarray, np.ndarray]:
        ...
        for i, (xyxy, _, confidence, class_id, tracker_id) in enumerate(detections):
            ...
            # Verifica si al menos una esquina del cuadro delimitador cruzó la línea
            if any(triggers):
                tracker_state = triggers[0]

                # Si el ID del rastreador no está en el diccionario, agrégalo y continúa
                if tracker_id not in self.tracker_state:
                    self.tracker_state[tracker_id] = tracker_state
                    continue

                # Si el estado del rastreador no ha cambiado, continúa
                if self.tracker_state.get(tracker_id) == tracker_state:
                    continue

                # Actualiza el estado del rastreador en el diccionario
                self.tracker_state[tracker_id] = tracker_state

                # Incrementa el contador correspondiente y registra la clase cuando cruza la línea
                if tracker_state:
                    lista = []
                    self.in_count += 1
                    class_id_when_crossed = class_id
                    lista.append(1)
                    lista.append(class_id_when_crossed)
                    return lista
    ...
    def trigger(self, detections: Detections) -> Tuple[np.ndarray, np.ndarray]:
        ...
        for i, (xyxy, _, confidence, class_id, tracker_id) in enumerate(detections):
            ...
            obj_center_point = Point(x=obj_center_x, y=obj_center_y)

            if self.vector.is_in(obj_center_point):  # Objeto está dentro de la zona de interés
                if not self.tracker_state.get(tracker_id, False):  # Objeto acaba de entrar
            ...
            else:  # Objeto está fuera de la zona de interés
                if self.tracker_state.get(tracker_id, False):  # Objeto acaba de salir
